...
def process_image(image_path, save_dir, bot, model, labels, api_tokens, cam_type, prompt, api_index=0, slope=25, position=0.6, model_type='classification'):
...
def generate_final_report(results):
    ...
    for result in results:
        is_correct = result['prediction'] == result['label']
...
def process_folder(input_folder, save_dir, model_name='resnet18', model=None, api_key=None, threshold=2.5, cam_type='gradcam', vlm_model='gemini/gemini-1.5-flash', prompt=1, slope=25, position=0.6, model_type='classification'):
    """
    ...
    if model is None:
        ...
        if hasattr(model, model_name):
            model = getattr(model, model_name)(pretrained=True).eval()
        else:
            raise ValueError(f"Model must be one of {', '.join(model.__dict__.keys())}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, default='classification', choices=['classification', 'segmentation'], help='Model type to use')
